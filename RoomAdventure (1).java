...
//          Rajan - Added the feature if the player reach room 8, player find the exit and win the game.
...
public class RoomAdventure { // Main class containing game logic
    ...
    private static Room winningRoom;
    private static Room deathRoom; // entering this room kills the player
    // stepping here shows the win banner
    ...
    // ─── 2. Drop this helper anywhere in RoomAdventure (e.g. under handleUse) ──
    private static void checkWin() {
        if (currentRoom == winningRoom && !gameWon) {
            gameWon = true;
            System.out.println("\n★  Congratulations!  You found the exit and won the game! ★");
            System.exit(0); // terminate program cleanly
        }
    }
    ...
    private static void handleDeath() {
        System.out.println(
                "\n☠️  A chill runs down your spine as the door slams shut… you are DEAD! ☠️\n");
        Scanner s = new Scanner(System.in);
        while (true) {
            System.out.print("Type 'restart' to play again or 'exit' to quit: ");
            String choice = s.nextLine().trim().toLowerCase();
            if (choice.equals("restart")) {
                restartGame();
                break; // back to the main game loop
            } else if (choice.equals("exit")) {
                System.out.println("Thanks for playing!");
                System.exit(0);
            } else {
                System.out.println("Please enter only 'restart' or 'exit'.");
            }
        }
    }

    // ==== Player win handler (non‑terminating) ===================================
    // ==== Player win handler (offers two artifacts, then exits) ================
    private static void handleWin() {
        if (gameWon)
            return; // show this menu only once
        gameWon = true;

        System.out.println("\n★  You have uncovered the treasure chamber! ★");
        System.out.println("Before you rest two artifacts—choose one to claim your fate:");
        System.out.println("  1) ✨ Crystal Orb of Insight");
        System.out.println("  2) ⚔️  Blade of Destiny");

        Scanner s = new Scanner(System.in);
        while (true) {
            System.out.print("Type 1 or 2 to choose your artifact: ");
            String choice = s.nextLine().trim();

            if (choice.equals("1") || choice.equalsIgnoreCase("orb")
                    || choice.equalsIgnoreCase("crystal")) {

                System.out.println(
                        "\nYou grasp the Crystal Orb. Visions swirl around you—\n"
                                + "your quest is complete. You WIN!\n");
                System.exit(0);

            } else if (choice.equals("2") || choice.equalsIgnoreCase("blade")
                    || choice.equalsIgnoreCase("sword")) {

                System.out.println(
                        "\nYou brandish the Blade of Destiny. Power surges through you—\n"
                                + "your quest is complete. You WIN!\n");
                System.exit(0);
    ...
    private static void restartGame() {
        setupGame(); // rebuild all rooms/items
        Arrays.fill(inventory, null); // empty pockets
        Stamina = 3;
        status = "You awaken at the entrance once more…";
    }

    // ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\
    // Method: setupGame
    // Purpose: Initializes the game world by creating and configuring all rooms,
    // their connections, items, item descriptions, and grabbables
    // ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\
...
// ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\
// Class: Room
// Purpose: Defines a room with exits, directions, destinations, items,
// grabbables, and descriptions.
// ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\
